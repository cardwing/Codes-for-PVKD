            ...
            if True:
                ...
                with torch.no_grad():
                    for i_iter_test, (_, _, test_grid, _, test_pt_fea, test_index, origin_len) in enumerate(
                        ...
                        predict_labels = my_model(test_pt_fea_ten, test_grid_ten, val_batch_size, test_grid, voting_num=4, use_tta=True)#val_batch_size)
                        ...
                        #for count, i_test_grid in enumerate(test_grid):
                            #test_pred_label = predict_labels[count,test_grid[count][:,0],test_grid[count][:,1],test_grid[count][:,2]][0:origin_len[count]]
                            ...
                            test_pred_label2 = test_pred_label[0:origin_len[0]]
                            test_pred_label2.tofile(new_save_dir)
                        pbar.update(1)
